...
function EFFECT:Init(data)
	...
	local owent
	local parent = self.WeaponEnt:GetParent()

	if IsValid(self.WeaponEnt) then
		owent = self.WeaponEnt:GetOwner()
	end

	if not IsValid(owent) and IsValid(parent) then
	...
	ParticleEffectAttach("bo3_raygun_muzzleflash", PATTACH_POINT_FOLLOW, self.WeaponEnt, self.Attachment)    
end
