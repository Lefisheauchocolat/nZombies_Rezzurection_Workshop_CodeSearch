...
function EFFECT:Init(data)
	...
	if self.WeaponEntOG:IsCarriedByLocalPlayer() && !LocalPlayer():ShouldDrawLocalPlayer() then
		ParticleEffectAttach("bo3_paralyzer_muzzleflash", PATTACH_POINT_FOLLOW, self.WeaponEnt, self.Attachment)
	else
		ParticleEffectAttach("bo3_paralyzer_muzzleflash_3p", PATTACH_POINT_FOLLOW, self.WeaponEnt, self.Attachment)
	end
end
