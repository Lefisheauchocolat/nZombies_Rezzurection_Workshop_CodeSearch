...
function EFFECT:Init(data)
	...
	if dlight_cvar:GetBool() then
		...
		if IsValid(self.WeaponEntOG) and self.WeaponEntOG:GetSilenced() then
			mult = 2
		end
		...
		if (dlight) then
			...
			dlight.r = self.random == 1 and 100 or 255
			dlight.g = self.random == 1 and 255 or 50
			dlight.b = self.random == 1 and 60 or 50
			dlight.brightness = 1*mult
			dlight.Decay = 3000
			dlight.Size = 200*mult
			dlight.DieTime = CurTime() + fadeouttime
		end
	end

	ParticleEffectAttach("waw_xray_muzzleflash_"..self.random, PATTACH_POINT_FOLLOW, self.WeaponEnt, self.Attachment)    
end
