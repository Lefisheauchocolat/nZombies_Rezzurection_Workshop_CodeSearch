...
function SWEP:PrimaryAttack()
	if self.CanChangeWeapon then
		...
		if !nzKillstreak:CheckLimit("vtol") then
			if SERVER then
				ply:EmitSound("ambient/levels/prison/radio_random5.wav", 60)
				ply:ChatPrint("VTOL Jet already in air!")
			end
			return
		end
		if !tr.HitSky then
			if SERVER then
				ply:EmitSound("ambient/levels/prison/radio_random5.wav", 60)
				ply:ChatPrint("VTOL Jet cant be called under roof!")
			end
			return
		end
		...
		timer.Simple(1, function()
			...
			timer.Simple(3, function()
				...
				local ent = ents.Create("bo6_vtol")
...
if CLIENT then
	...
	hook.Add("PostDrawTranslucentRenderables", "nzrVTOLMarker", function()
		local ply = LocalPlayer()
		if ply:Alive() and IsValid(ply:GetActiveWeapon()) and ply:GetActiveWeapon():GetClass() == "bo6_killstreak_vtol" and ply:GetActiveWeapon().CanChangeWeapon then
