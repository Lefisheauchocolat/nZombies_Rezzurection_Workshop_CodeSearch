...
function SWEP:SetupPlaceable(vm)	
	...
	local tr1 = util.QuickTrace(pos, aim:Forward() * self.PlaceDistance, {ply, self})
	local dot = tr1.HitNormal:Dot(Vector(0,0,1))

	if tr1.HitWorld and (dot <= 0.5) and (dot >= 0) then
		local test = tr1.HitNormal:Angle() + (self:GetFlipped() and Angle(-90,180,0) or Angle(90,0,0))
		test:RotateAroundAxis(tr1.HitNormal, test.z - tr1.HitNormal:Angle().z)

		return {tr1.HitPos, test, false}
	end
	...
	if tr.HitWorld then
		return {tr.HitPos, ang, false}
	end

	return {ply:GetPos(), ang, false}
end
