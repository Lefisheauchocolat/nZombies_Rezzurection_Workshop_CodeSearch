...
function SWEP:PostPrimaryAttack()
	...
	if self:VMIV() and ifp then
		...
		if self.Ispackapunched then
			ParticleEffectAttach("bo4_mk2_vm_pap", PATTACH_POINT_FOLLOW, self.OwnerViewModel, 3)
			ParticleEffectAttach("bo4_mk2_vm_pap", PATTACH_POINT_FOLLOW, self.OwnerViewModel, 4)
		else
			ParticleEffectAttach("bo4_mk2_vm", PATTACH_POINT_FOLLOW, self.OwnerViewModel, 3)
			ParticleEffectAttach("bo4_mk2_vm", PATTACH_POINT_FOLLOW, self.OwnerViewModel, 4)
		end
	end
end
	...
	if DynamicLight then
		if dlight_cvar:GetBool() then
			...
			if self.DLight then
				...
				local ourpos = (attpos and attpos.Pos) and attpos.Pos or self:GetPos()
				if attpos then
					ourpos = ourpos - attpos.Ang:Forward()*14
					ourpos = ourpos - attpos.Ang:Right()*2
				end
