...
SWEP.WW3P_ATT = 5
SWEP.WW3P_CALLBACK = function(wep, ply)
	if !wep.CL_3PDrawFXb or !wep.CL_3PDrawFXb:IsValid() then
		wep.CL_3PDrawFXb = CreateParticleSystem(wep, "bo4_dg5_3p", PATTACH_POINT_FOLLOW, 6)
	end

	local WorldModelElements = wep:GetStatRaw("WorldModelElements", TFA.LatestDataVersion)
	if WorldModelElements and WorldModelElements['dg5_dw'] and IsValid(WorldModelElements['dg5_dw'].curmodel) then
		local wmodel = WorldModelElements['dg5_dw'].curmodel
		if !wep.CL_3PDrawFXc or !wep.CL_3PDrawFXc:IsValid() then
			wep.CL_3PDrawFXc = CreateParticleSystem(wmodel, "bo4_dg5_3p", PATTACH_POINT_FOLLOW, 5)
		end
		if !wep.CL_3PDrawFXd or !wep.CL_3PDrawFXd:IsValid() then
			wep.CL_3PDrawFXd = CreateParticleSystem(wmodel, "bo4_dg5_3p", PATTACH_POINT_FOLLOW, 6)
		end
	end
end
SWEP.WW3P_REMOVECALLBACK = function(wep, ply)
	if wep.CL_3PDrawFXb and wep.CL_3PDrawFXb:IsValid() then
		wep.CL_3PDrawFXb:StopEmissionAndDestroyImmediately()
	end
	if wep.CL_3PDrawFXc and wep.CL_3PDrawFXc:IsValid() then
		wep.CL_3PDrawFXc:StopEmissionAndDestroyImmediately()
	end
	if wep.CL_3PDrawFXd and wep.CL_3PDrawFXd:IsValid() then
		wep.CL_3PDrawFXd:StopEmissionAndDestroyImmediately()
	end
end
...
function SWEP:StartFirstPersonParticles()
	...
	if !self.WWFP_FX or !IsValid(self.WWFP_FX) then
		self.WWFP_FX = CreateParticleSystem(self.OwnerViewModel, "bo4_dg5_vm", PATTACH_POINT_FOLLOW, 3)
	end
	if !self.WWFP_FXb or !IsValid(self.WWFP_FXb) then
		self.WWFP_FXb = CreateParticleSystem(self.OwnerViewModel, "bo4_dg5_vm", PATTACH_POINT_FOLLOW, 4)
	end
	if !self.WWFP_FXc or !IsValid(self.WWFP_FXc) then
		self.WWFP_FXc = CreateParticleSystem(self.OwnerViewModel, "bo4_dg5_vm", PATTACH_POINT_FOLLOW, 5)
	end
	if !self.WWFP_FXd or !IsValid(self.WWFP_FXd) then
		self.WWFP_FXd = CreateParticleSystem(self.OwnerViewModel, "bo4_dg5_vm", PATTACH_POINT_FOLLOW, 6)
	end
end
...
function SWEP:StopFirstPersonParticles()
	...
	if self.WWFP_FXb and IsValid(self.WWFP_FXb) then
		self.WWFP_FXb:StopEmissionAndDestroyImmediately()
	end
	if self.WWFP_FXc and IsValid(self.WWFP_FXc) then
		self.WWFP_FXc:StopEmissionAndDestroyImmediately()
	end
	if self.WWFP_FXd and IsValid(self.WWFP_FXd) then
		self.WWFP_FXd:StopEmissionAndDestroyImmediately()
	end
end
...
function SWEP:PrimaryAttack()
	...
	if shouldDisplayDebug() and self:Clip1()%5 == 0 then
		local end_pos = ply:GetShootPos() + (fwd*120) + ply:OBBCenter()
...
function SWEP:Think2(...)
	...
	if CLIENT and dlight_cvar:GetBool() and DynamicLight then
		...
		if self.DLight then
			self.DLight.pos = ply:GetShootPos()
			self.DLight.r = 45
			self.DLight.g = 165
			self.DLight.b = 255
			...
			for k, v in pairs(ents.FindInSphere(ply:GetShootPos(), 64)) do
				if (v:IsNPC() or v:IsNextBot()) and v:Health() > 0 then
					...
					if not v:BO4IsShocked() then
						util.ParticleTracerEx("bo3_waffe_jump", ply:GetShootPos()-Vector(0,0,10), v:EyePos(), false, ply:EntIndex(), v:EntIndex())
					end
...
function SWEP:CanAltAttack()
	...
	return tr.HitWorld and self:CanPrimaryAttack() and self:Clip1() >= self:GetStatL("Secondary.AmmoConsumption")
end
...
function SWEP:AOEShock()
	...
	if self:VMIV() then
		...
		if CLIENT then
			if !self.DG5FP_A or !self.DG5FP_A:IsValid() then
				self.DG5FP_A = CreateParticleSystem(self.OwnerViewModel, "bo4_dg5_vm_charged", PATTACH_POINT_FOLLOW, 7)
			end
			if !self.DG5FP_B or !self.DG5FP_B:IsValid() then
				self.DG5FP_B = CreateParticleSystem(self.OwnerViewModel, "bo4_dg5_vm_charged", PATTACH_POINT_FOLLOW, 8)
			end
		end
	end
