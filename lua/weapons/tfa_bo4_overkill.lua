...
SWEP.Purpose = "Adopted by Dempsey with love and affection from a Group 935 armory, the Overkill mini-gun can chop meat and cut daisies with the greatest of ease."
SWEP.Author = "FlamingFox"
SWEP.Slot = 0
SWEP.PrintName = nzombies and "Overkill | BO4" or "Overkill"
...
--[Model]--
SWEP.ViewModel			= "models/weapons/tfa_bo4/overkill/c_overkill.mdl"
SWEP.ViewModelFOV = 65
SWEP.WorldModel			= "models/weapons/tfa_bo4/overkill/w_overkill.mdl"
...
--[Gun Related]--
SWEP.Primary.Sound = "TFA_BO4_OVERKILL.Start"
SWEP.Primary.LoopSound = "TFA_BO4_OVERKILL.Loop"
SWEP.Primary.LoopSoundTail = "TFA_BO4_OVERKILL.Stop"
SWEP.Primary.Ammo = "none"
SWEP.Primary.Automatic = true
SWEP.Primary.RPM = 1200
SWEP.Primary.RPM_Semi = nil
SWEP.Primary.RPM_Burst = nil
SWEP.Primary.Damage = nzombies and 115 or 30
...
SWEP.Primary.ProjectileModel    = "models/weapons/tfa_bo4/overkill/overkill_projectile.mdl" -- Entity to shoot's model
...
SWEP.NZHudIcon = Material("vgui/icon/ui_icon_equipment_zm_minigun_lvl3_dark.png", "unlitgeneric smooth")
...
function SWEP:OnSpecialistRecharged()
	...
	if SERVER then
		self:GetOwner():SetAmmo(self.Secondary.DefaultClip, self:GetSecondaryAmmoType())
	end
end
...
{ ["time"] = 1 / 30, ["type"] = "sound", ["value"] = Sound("TFA_BO4_OVERKILL.Draw") },
{ ["time"] = 1 / 30, ["type"] = "sound", ["value"] = Sound("TFA_BO4_OVERKILL.Deploy") },
},
[ACT_VM_DRAW] = {
{ ["time"] = 1 / 30, ["type"] = "sound", ["value"] = Sound("weapon_bo3_cloth.heavy") },
{ ["time"] = 5 / 30, ["type"] = "sound", ["value"] = Sound("TFA_BO4_OVERKILL.Pullout") },
},
[ACT_VM_HOLSTER] = {
{ ["time"] = 2 / 30, ["type"] = "sound", ["value"] = Sound("weapon_bo3_cloth.heavy") },
{ ["time"] = 5 / 30, ["type"] = "sound", ["value"] = Sound("TFA_BO4_OVERKILL.Holster") },
},
[ACT_VM_HITCENTER] = {
{ ["time"] = 0, ["type"] = "sound", ["value"] = Sound("TFA_BO4_OVERKILL.Throw") },
},
}
...
SWEP.Secondary.Sound = "TFA_BO4_OVERKILL.ShootGL"
...
SWEP.Glow = Material("models/weapons/tfa_bo4/overkill/mtl_wpn_t8_zm_minigun_barrels.vmt")
...
function SWEP:SetupDataTables(...)
	...
	self:NetworkVarTFA("Bool", "Nuking")
end
...
function SWEP:PrimaryAttack(...)
	local ply = self:GetOwner()
	if IsValid(ply) and ply:IsNPC() and math.random(4) == 1 and (self.NPCLastSecondaryAttack + self.NPCSecondaryDelay) < CurTime() then
		local t_tr = util.QuickTrace(ply:EyePos(), ply:GetAimVector()*ply:GetMaxLookDistance(), {self, ply})
		if IsValid(t_tr.Entity) and t_tr.Entity == ply:GetEnemy() then
...
function SWEP:AltAttack()
	...
	if self:GetStatus() ~= TFA.Enum.STATUS_BASHING and TFA.Enum.ReadyStatus[self:GetStatus()] and not self:GetSprinting() then
		self:StopSound("TFA_BO4_OVERKILL.SpinLoop")
		self:StopSoundNet("TFA_BO4_OVERKILL.SpinLoop")
...
function SWEP:TacticalNuke()
	local ang = self:GetAimVector():Angle()
	local pos = self:GetOwner():GetShootPos()

	if CLIENT then return end
	
	local ent = ents.Create("bo4_specialist_overkill_nuke")
...
function SWEP:CanSecondaryAttack()
	...
	if self:GetStatL("Primary.ClipSize") <= 0 and self:Ammo1() < self:GetStatL("Secondary.AmmoConsumption") then
		return false
	end

	if self:GetPrimaryClipSize(true) > 0 and self:Clip1() < self:GetStatL("Secondary.AmmoConsumption") then
		return false
	end
...
function SWEP:Think2(...)
	...
	if self:GetGlowLevel() > 0 and status ~= TFA.Enum.STATUS_SHOOTING then
		self:SetGlowLevel(math.Approach(self:GetGlowLevel(), 0, FrameTime()))
		self.Glow:SetFloat("$emissiveblendstrength", self:GetGlowLevel())
	end

	if status == TFA.Enum.STATUS_BASHING then
		if self:GetStatusProgress() >= 0.5 and not self:GetHasNuked() then
			self:SetHasNuked(true)
			self:TacticalNuke()
		end
		if CurTime() > self:GetStatusEnd() then
			self:SetNuking(false)

			if nzombies then
				self:SetClip1(math.min(self:Clip1(), self:GetStatL("Primary.ClipSize")/2))
				ply:SetUsingSpecialWeapon(false)
				ply:EquipPreviousWeapon()
			else
				self:SendViewModelAnim(ACT_VM_DRAW_DEPLOYED)
				self:ScheduleStatus(TFA.Enum.STATUS_DRAW , self:GetActivityLength())
				self:SetNextPrimaryFire(self:GetStatusEnd())
			end
		end
	end
	...
	if CLIENT and ply:IsPlayer() then
		if ply:KeyDown(IN_ATTACK) and self:CanSpinUp() then
	...
	if IsValid(ply) and ply:IsPlayer() then
		if ply:KeyDown(IN_ATTACK) and self:CanSpinUp() then
			if not self:GetIsSpinning() then
				if IsFirstTimePredicted() then self:EmitSoundNet("TFA_BO4_OVERKILL.SpinStart") end
			end
			...
			if l_CT() > self:GetNextPrimaryFire() or self.Primary.Automatic then
				if self:GetIsSpinning() then
					if IsFirstTimePredicted() then self:EmitSoundNet("TFA_BO4_OVERKILL.SpinStop") end
				end
		...
		if self:GetIsSpinning() then
			...
			if self:GetSpinTime() >= 1 then
				self:EmitSoundNet("TFA_BO4_OVERKILL.SpinLoop")
...
function SWEP:SecondaryAttack()
	...
	self.Primary_TFA.Projectile = "bo4_specialist_overkill"
	self.Primary_TFA.NumShots = 1
	self.Primary_TFA.AmmoConsumption = inf_cvar:GetBool() and 0 or 10

	self:TriggerAttack("Secondary", 1)

	self:SetSilenced(false)
	self.Primary_TFA.Projectile = nil
	self.Primary_TFA.NumShots = 3
	self.Primary_TFA.AmmoConsumption = inf_cvar:GetBool() and 0 or 1
...
function SWEP:PostPrimaryAttack()
	self:SetGlowLevel(math.Approach(self:GetGlowLevel(), 3, 0.02))
	self.Glow:SetFloat("$emissiveblendstrength", self:GetGlowLevel())
end
...
function SWEP:OnDrop(...)
	self:StopSound("TFA_BO4_OVERKILL.Loop")
	self:StopSound("TFA_BO4_OVERKILL.SpinLoop")
...
function SWEP:Holster(...)
	if self:GetNuking() then
		return false
	end

	self:StopSound("TFA_BO4_OVERKILL.Loop")
	self:StopSound("TFA_BO4_OVERKILL.SpinLoop")
	self:StopSoundNet("TFA_BO4_OVERKILL.Loop")
	self:StopSoundNet("TFA_BO4_OVERKILL.SpinLoop")
	self.Glow:SetFloat("$emissiveblendstrength", 0)
	return BaseClass.Holster(self,...)
end

local crosshair_pineapple = Material("vgui/overlay/reticle_pineapplegun.png", "smooth unlitgeneric")
local CMIX_MULT = 1
local c1t = {}
local c2t = {}
...
function SWEP:DrawHUDBackground() //copy pasting the entire function to change 1 thing B)
	...
	surface.SetMaterial(crosshair_pineapple)
