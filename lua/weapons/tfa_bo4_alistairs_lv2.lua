...
SWEP.Slot = 1
SWEP.PrintName = nzombies and "Chaos Theory | BO4" or "Chaos Theory"
SWEP.DrawCrosshair = true
SWEP.DrawCrosshairIS = false

--[Model]--
SWEP.ViewModel			= "models/weapons/tfa_bo4/alistairs/c_alistairs_lvl2.mdl"
SWEP.ViewModelFOV = 65
SWEP.WorldModel			= "models/weapons/tfa_bo4/alistairs/w_alistairs_lvl2.mdl"
...
SWEP.Primary.Damage = nzombies and 800 or 100
...
SWEP.MuzzleFlashEffect	= "tfa_bo4_muzzleflash_alistairs_pap"
...
SWEP.IronSightsPos = Vector(-5.23, 0, -0.55)
...
--[NZombies]--
SWEP.NZPaPName = "Chaos Theory"
SWEP.NZPaPReplacement = "tfa_bo4_alistairs_lv3"
...
SWEP.VElements = {
	["sprite"] = { type = "Sprite", sprite = "sprites/animglow01-", bone = "tag_fx_light", rel = "", pos = Vector(0,0,0), size = {x = 0.5, y = 0.5}, color = Color(50, 255, 200, 150), nocull = true, additive = true, vertexalpha = true, vertexcolor = true, ignorez = true, active = true },
}
		...
		if TFA.Enum.ReadyStatus[self:GetStatus()] and not self:GetSprinting() then
			if self:GetCurrentCharge() == self.ChargeMaxTime and not self.HasPlayedChargeSound then
				...
				if IsValid(self) and self:VMIV() then
					if IsFirstTimePredicted() then ParticleEffectAttach("bo4_chaos_vm_light", PATTACH_POINT_FOLLOW, self.OwnerViewModel, 3) end
				end
			end
...
function SWEP:LaunchProjectile()
	...
	if self:GetCurrentCharge() == self.ChargeMaxTime then
		if self:GetChargeType() >= 2 then self:SetChargeType(0) end
...
function SWEP:PrimaryAttack()
	...
	if self:GetChargeStart() <= 0 then
		...
		if IsValid(self) and self:VMIV() then
			if IsFirstTimePredicted() then ParticleEffectAttach("bo4_chaos_vm_glow", PATTACH_POINT_FOLLOW, self.OwnerViewModel, 5) end
		end
