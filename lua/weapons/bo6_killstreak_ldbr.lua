	...
	if self.CanChangeWeapon then
		...
		if !tr.HitSky then
			if SERVER then
				...
				ply:ChatPrint("LDBR cant be called under roof!")
			end
			return
		end
		...
		timer.Simple(1, function()
			...
			self:Remove()
			local ent = ents.Create("bo6_ldbr_marker")
			ent:SetPos(pos)
			ent:SetOwner(ply)
			ent:SetNoDraw(true)
			ent:Spawn()
...
if CLIENT then
	...
	hook.Add("PostDrawTranslucentRenderables", "nzrLDBRMarker", function()
		local ply = LocalPlayer()
		if ply:Alive() and IsValid(ply:GetActiveWeapon()) and ply:GetActiveWeapon():GetClass() == "bo6_killstreak_ldbr" and ply:GetActiveWeapon().CanChangeWeapon then
