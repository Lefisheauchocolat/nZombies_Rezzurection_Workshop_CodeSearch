...
function mapscript.shootable()
	...
	for k,v in pairs(buttons) do
		local button
		local usable = true
		if IsValid(buttons_ents[k]) then
			shootable = buttons_ents[k]
		else
			shootable = ents.Create("nz_script_prop")
		end
		...
		button:SetModel("models/cheeze/buttons2/pwr_red.mdl")
		button:Spawn()
		button:EmitSound( "buttons/button18.wav", 100, 100, 1, CHAN_AUTO )
		security.OnUsed = function(self, ply)
		if usable then
			usable = false
			security:SetNWString("NZText", "")
			security:EmitSound( "buttons/button18.wav", 100, 100, 1, CHAN_AUTO )
			mapscript.securitylight()
		end
		button.OnTakeDamage = function(self, dmginfo)
			local attacker = dmginfo:GetAttacker()
			if attacker:IsPlayer() then
				--ShootableCount = ShootableCount + 1
				attacker:SendLua( "surface.PlaySound( \"nz_moo/effects/buy_t9.mp3\" ) " )
