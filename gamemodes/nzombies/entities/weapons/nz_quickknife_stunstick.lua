if SERVER then
	AddCSLuaFile("nz_quickknife_stunstick.lua")
	SWEP.Weight			= 5
...
function SWEP:PrimaryAttack()
	...
	if IsValid(ply) then
		...
		local trace			= {}
			...
			trace.endpos	= ply:GetShootPos() + ( ply:GetAimVector() * self.Range)
			trace.filter	=   function( ent )
									if ent == ply or invalident[ent:GetClass()] then
										return false
									else
										return true
									end
		...
		if IsValid(traceHit.Entity) and traceHit.Entity:IsValidZombie() or traceHit.Hit and traceHit.HitWorld then
			...
			return
		elseif IsValid(traceHit.Entity) and valident[traceHit.Entity:GetClass()] then
			...
			if ( SERVER ) then
				ply:TraceHullAttack( ply:GetShootPos(), traceHit.HitPos, Vector( -5, -5, -5 ), Vector( 5, 5, 5), self.Primary.Damage, self.Primary.DamageType, self.Primary.Force )
			end
