...
ENT.PrintName = "Xenomorph Spitter"
ENT.Category = "Brainz"
ENT.Author = "Wavy"
ENT.Spawnable = true
...
function ENT:PostAdditionalZombieStuff()
	...
	if self:TargetInRange(200) and !self:IsAttackBlocked() and self.CanJump and !nzPowerUps:IsPowerupActive("timewarp") then
		if !self:GetTarget():IsPlayer() then return end
		self:JumpAttack()
	end
	if self:TargetInRange(850) and !self:IsAttackBlocked() and self.CanSpit and !nzPowerUps:IsPowerupActive("timewarp") and math.random(3) == 3 then
		if !self:GetTarget():IsPlayer() then return end
