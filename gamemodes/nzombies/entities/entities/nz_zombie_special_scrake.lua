	...
	function ENT:EffectsAndSounds()
		if self:Alive() then
			...
			if (!self.Draw_FX or !IsValid(self.Draw_FX)) then
				self.Draw_FX = CreateParticleSystem(self, "bo2_flinger_leak_smoke3", PATTACH_POINT_FOLLOW, 2)
	...
	if SERVER then
		...
		if nzRound:InState( ROUND_CREATE ) then
			...
			if nzRound:InState( ROUND_PROG ) then
				self:SetHealth(math.Clamp(nzRound:GetNumber() * 500, 2500, 30000 * count))
				self:SetMaxHealth(math.Clamp(nzRound:GetNumber() * 500, 2500, 30000 * count))
			else
				self:SetHealth(2500)
		...
		self:SetSurroundingBounds(Vector(-28, -28, 0), Vector(28, 28, 95)) 	-- Nextbots Surrounding Bounds(For Hitbox detection.)
