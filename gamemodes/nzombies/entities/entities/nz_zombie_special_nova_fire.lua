...
ENT.Type = "nextbot"
ENT.PrintName = "Höllen Kriecher"
ENT.Category = "Brainz"
ENT.Author = "GhostlyMoo"
ENT.Spawnable = true

function ENT:InitDataTables()
	self:NetworkVar("Bool", 6, "Enraged")
end
...
if CLIENT then
	...
	function ENT:EffectsAndSounds()
		if self:IsAlive() then
			...
			if (!self.Draw_FX or !self.Draw_FX:IsValid()) and self:GetEnraged() then
				self.Draw_FX = CreateParticleSystem(self, "doom_hellunit_aura", PATTACH_POINT_FOLLOW, 9)
			end
		end
	end
	return 
...
ENT.Models = {
	{Model = "models/moo/_codz_ports/codol/quad/moo_codz_codol_devil_crawler.mdl", Skin = 0, Bodygroups = {0,0}},
}
	...
	{Threshold = 300, Sequences = {
		{
			...
			SpawnSequence = {spawnfast},
			CrawlOutSequences = {crawlspawnsprint},
			StandAttackSequences = {SuicideAttackSequences},
			AttackSequences = {SuicideAttackSequences},
			JumpSequences = {JumpSequences},
			PassiveSounds = {walksounds},
		},
	}},
...
function ENT:StatsInitialize()
	if SERVER then
		...
		if speeds then
			self:SetRunSpeed( math.Clamp(nzMisc.WeightedRandom(speeds) + math.random(0,35), 0, 200) )
		else
...
function ENT:AI()
	...
	-- Hell Nova Logic
	if self.ImMad < 15 and !self:GetEnraged() then
		self:SetEnraged(true)
		self:SetRunSpeed(300)
		self:SpeedChanged()

		self:SetCollisionBounds(Vector(-6,-6, 0), Vector(6, 6, 72))
		self:SetSurroundingBounds(Vector(-32, -32, 0), Vector(32, 32, 74))

		self.ZombieLandSequences = {
			"nz_base_traverse_procedural_land",
		}
	end
...
function ENT:PostDeath(dmginfo)
	if math.random(2) == 2 or self:GetEnraged() then
		...
		ParticleEffect("spit_impact_orange", self:GetPos() + Vector(0,0,35), Angle(0,0,0), nil) 
...
function ENT:CustomAnimEvent(a,b,c,d,e) 
	...
	if e == "anim_explosive_zombie_death_tell_quick" then
		self.BoutaBlow = true
		self:PlaySound(self.SpawnSounds[math.random(#self.SpawnSounds)], 85, math.random(85,105), 1, 2)
	end
