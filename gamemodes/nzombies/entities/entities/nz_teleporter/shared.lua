...
ENT.GifTextures = { -- Give it all the same order as in sh_tools_teleporter
	[1] = {mat = "codtele", title = "Der Riese"},
	[2] = {mat = "coldwartp", title = "Cold War"},
	[3] = {mat = "bo3tp", title = "Black Ops 3"},
	[4] = {mat = "soe", title = "Shadows of Evil"},
	[5] = {mat = "originstp", title = "Origins (Black Ops 3)"}
}
...
function ENT:GetGif()
	return self.GifTextures[self:GetGifType()] and self.GifTextures[self:GetGifType()].mat or "codtele"
end
...
function ENT:Teleport() -- Start the teleportation procedure
	...
	for _,ply in pairs(self:GetTeleportingPlayers()) do
		...
		ply:SendLua([[surface.PlaySound("teleport.mp3")]])
	end
	...
	timer.Create("NZRTeleporterInitialize" .. self:EntIndex(), 4, 1, function() -- Time until the locked players are actually teleported
		if IsValid(self) and IsValid(tp_destination) then
			...
			self:EmitSound("weapons/tfa_waw/teslanade/teleport_out.wav", 100, math.random(95,105))
	
			tp_destination:EmitSound("weapons/tfa_waw/teslanade/teleport_out.wav", 100, math.random(95,105))
	
			ParticleEffect("driese_tp_departure_phase2",tp_destination:GetPos()+Vector(0,0,20),tp_destination:GetAngles(),self)
			...
			if (anyPlys and self:GetTPBack()) then
				...
				for _, ply in pairs(self:GetTeleportingPlayers()) do
					if IsValid(ply) then
						ply:SetNW2Float("nzStopWatch", CurTime() + self:GetTPBackDelay())
					end
				end
...
function ENT:Use(activator, caller)
	...
	if (IsValid(activator) and (activator:IsPlayer() and (self:IsOn() or activator:IsInCreative()))) then
		...
		if (activator:IsInCreative() or (activator:GetPoints() >= price and !self:GetOnCooldown())) then
			...
			timer.Simple(self:GetTeleporterTime() - 1, function()
				if IsValid(self) then
					...
					effectData:SetEntity(nil)
					util.Effect("lightning_prespawn", effectData)]]
					self:EmitSound("weapons/tfa_waw/teslanade/warmup.wav", 100, math.random(95,105))

					ParticleEffect("driese_tp_departure_phase1",self:GetPos()+Vector(0,0,20),self:GetAngles(),self)
