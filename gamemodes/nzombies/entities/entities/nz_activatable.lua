...
function ENT:Deactivation(creativeMode)
	...
	if (!creativeMode) then
		if time >= 90000 then return end -- If its this much... Then don't bother cooling down.
...
function ENT:Use(act, caller, type, value )
	...
	if IsValid(caller) and caller:IsPlayer() and not self:IsRemoteActivated() and not self:IsCooldownActive() and not self:IsActive() then
		if caller:CanAfford(self:GetCost()) then
			...
			self:EmitSound("buttons/lever7.wav", 100, math.random(99, 101))
			if SERVER then
...
function ENT:GetNZTargetText()
	...
	if self:IsElectricityNeeded() and not nzElec:IsOn() then
		...
		if self:GetCost() > 0 then
			return (!self:GetDisplayName() or #self:GetNZName() == 0) and "Press E - Activate Trap (Cost: " .. self:GetCost() .. " )" or "Press E - Activate " .. self:GetNZName() .. " (Cost: " .. self:GetCost() .. " )"
		else
			return (!self:GetDisplayName() or #self:GetNZName() == 0) and "Press E - Activate Trap" or usekey .. "Press E - Activate " .. self:GetNZName() .. "."
		end
	end
end
