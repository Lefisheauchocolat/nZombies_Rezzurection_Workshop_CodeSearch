...
if SERVER then
	...
	function ENT:Initialize()
		...
		self:SetCollisionBounds(Vector(-6,-6, 0), Vector(6, 6, 72))			-- Nextbots Collision Box(Mainly for interacting with the world.)
...
if SERVER then
	...
	function ENT:Think()
		if (self:IsOnGround() and !self:GetSpecialAnimation() and !self:GetIsBusy() and !self:GetCrawler() 
		and self.SameSquare and --[[self.loco:GetVelocity():Length2D() >= 125 and]] !nzPowerUps:IsPowerupActive("timewarp")) then -- Moo Mark
	...
	function ENT:StuckPrevention()
		...
		if !self:GetIsBusy() and !self:GetSpecialAnimation() and !self:GetAttacking() and self:GetLastPostionSave() + 4 < CurTime() then
			if self:GetPos():DistToSqr( self:GetStuckAt() ) < 10 then
				...
				if !self.SameSquare and !self:GetIsBusy() and self:GetStuckCounter() < 1 then
					self.SameSquare = true
				end
			end

			if self:GetStuckCounter() > 0 then
			...
			if self:GetStuckCounter() > 3 then
				...
				if !tr.HitNonWorld then
					self:ApplyRandomPush(1650) -- Made this higher so it actually PUSHES them and doesn't just breathe on them.
				end
	...
	function ENT:RunBehaviour()
		...
		while (true) do
			...
			if !self:GetStop() then
				...
				if !self:GetFleeing() and !self:GetWandering() and !self:GetStop() and CurTime() > self:GetLastFlee() + 1 then
					self:SetTimedOut(false)
					local ct = CurTime()
	...
	function ENT:ZCTInit()
		...
		if nzMapping.Settings.zct == 1 then
			if !self.SetZCTacoZombie then
				if self.IsMooSpecial or self.IsMooBossZombie or self.IsCatalyst or self.NZBossType then return end
			...
			if (IsValid(barricade) and barricade:GetClass() == "breakable_entry" ) then
				...
				if barricade:GetNumPlanks() > 0 then
					...
					if self:LookupSequence(result) > 0 and (self.IsMooSpecial and self.MooSpecialZombie or !self.IsMooSpecial) then
						self:SetPos(Vector(bpos.x,bpos.y,bpos.z + 5))
...
function ENT:OnThink()
	...
	-- Moo Mark 8/1/24: Moved to RunBehaviour, while the gestures tend to jitter and spaz more. Runbehaviour actually allows them to be 100% consistent in mp(FUCK YOU).
	--self:ZombieGestureSequences() -- Not every zombie has this feature so its not that important, just a nice little cosmetic addition.
...
if SERVER then
	...
	function ENT:BecomeCrawler() -- For turning into Crawlers.
		...
		self:SetCollisionBounds(Vector(-6,-6, 0), Vector(6, 6, 26))
	end
...
if SERVER then
	...
	function ENT:ChaseTarget( options )
		...
		local slowpth = 1.45 + self.RepathTimeMod
		local quickpth = 0.25 + self.RepathTimeMod
		...
		while ( path:IsValid() and self:HasTarget() and !self:TargetInAttackRange() and !self.Big_Jump_area_start ) do
			...
			if path:IsValid() and !self.CancelCurrentPath then -- This is pulled from Ba2 for distance based repathing.	
				...
				if (distToTarget > 850^2) then
					if path:GetAge() > math.Clamp(distToTarget / 1000^2,slowpth,15) then
						return "timeout"
					end
				else
					if path:GetAge() > math.Clamp(distToTarget / 295^2,quickpth,2) then -- We're closing in, let's start repathing sooner!
						return "timeout"
					end
				end
			else
	...
	function ENT:PerformIdle()
		...
		elseif (self.BO4IsShocked and self:BO4IsShocked() or self.BO4IsScorped and self:BO4IsScorped() or self.BO4IsSpinning and self:BO4IsSpinning() or self:GetNW2Bool("OnAcid") or self.BeingDissolved) and self:GetCrawler() and !self.IsMooSpecial then
			self:ResetSequence(self.SparkyCrawlAnim)
		elseif (self.BO4IsShocked and self:BO4IsShocked() or self.BO4IsScorped and self:BO4IsScorped() or self:GetNW2Bool("OnAcid") or self.BeingDissolved) and !self:GetCrawler() and !self.IsMooSpecial then
	...
	if options.collisions == nil then options.collisions = true end
		...
		local res = self:PlaySequenceAndWait(seq, options.rate, function(self, cycle)
			...
			if success then
				...
				if !tr.Hit and (qtr.Hit or self.Big_Jump_area_ledge == "reached") or (self:GetIsBusy() and (self.TraversalAnim or tr.HitNoDraw)) then
