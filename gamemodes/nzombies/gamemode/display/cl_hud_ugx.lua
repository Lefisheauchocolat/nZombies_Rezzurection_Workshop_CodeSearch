...
//t5 hud
local t5_hud_dpad_outer_frame = Material("nz_moo/huds/ugx/ugx_hand_hud.png", "unlitgeneric smooth")
...
local t5_hud_dpad_lines_fade = Material("nz_moo/huds/ugx/ugx_hud_bg.png", "unlitgeneric smooth")
...
local zmhud_icon_talk = Material("nz_moo/huds/ugx/talkballoon.png", "unlitgeneric smooth")
...
net.Receive("nz_points_notification_ugx", function()
...
local function GunHud_t5()
	...
	if IsValid(wep) then
		...
		elseif not illegalspecials[wep.NZSpecialCategory] then
			...
			if ply:HasPerk("mulekick") then
				...
				surface.DrawTexturedRect(w - 265*scale, h - 195*scale, 35*scale, 35*scale)
			end
		end
	end
	...
	if num > 0 then
		...
		for i = num, 1, -1 do
			...
			surface.DrawTexturedRect(w - 280*scale + i*(10*scale), h - 42*scale, 42*scale, 42*scale)
		end
	end
	...
	if numspecial > 0 then
		...
		for i = numspecial, 1, -1 do
			...
			surface.DrawTexturedRect(w - 340*scale + i*(10*scale), h - 42*scale, 42*scale, 42*scale)
		end
	end
end
...
local function PerksMMOHud_t5()
	...
	for k, v in pairs(ply:GetPerks()) do
		...
		elseif mmohud.style == "count" then
			draw.SimpleTextOutlined(ply:GetNW2Int(tostring(mmohud.count), 0), ChatFont, w - 230*scale - (40*traycount*scale), h - 155*scale, fontColor, TEXT_ALIGN_RIGHT, TEXT_ALIGN_BOTTOM, 1, color_black)
		elseif mmohud.style == "%" then
			...
			if (not mmohud.hide) or (mmohud.hide and perkpercent < 100) then
				draw.SimpleTextOutlined(perkpercent.."%", ChatFont, w - 230*scale - (40*traycount*scale), h - 155*scale, fontColor, TEXT_ALIGN_RIGHT, TEXT_ALIGN_BOTTOM, 1, color_black)
			end
		elseif mmohud.style == "chance" then
			draw.SimpleTextOutlined(ply:GetNW2Int(tostring(mmohud.count), 0).."/"..mmohud.max, ChatFont, w - 230*scale - (40*traycount*scale), h - 155*scale, fontColor, TEXT_ALIGN_RIGHT, TEXT_ALIGN_BOTTOM, 1, color_black)
		end
...
-- Hooks
hook.Add("HUDPaint", "nzHUDswapping_ugx", function()
	if nzMapping.Settings.hudtype == "UGX 1.1" then
...
//--------------------------------------------------/UGX Team, GhostlyMoo and Fox's ugx HUD\------------------------------------------------\\
