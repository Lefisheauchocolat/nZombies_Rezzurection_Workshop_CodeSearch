		...
		if wep.IsTFAWeapon and wep.NZSpecialCategory == "trap" then
			...
			if ammo > 0 and not wep.TrapCanBePlaced then
				...
				if ammocount > 0 then
					...
					surface.SetDrawColor(color_white)
					surface.DrawTexturedRect(w - 88*scale, h - 178*scale, 60*scale, 60*scale)
				end
			else
				surface.SetMaterial(t7_icon_trap_fill)
				surface.SetDrawColor(color_white)
...
local function PowerUpsHud_t7_zod()
	...
	for k,v in pairs(nzPowerUps.ActivePowerUps) do	
		if nzPowerUps:IsPowerupActive(k) then
			...
			if k == "timewarp" then
				AddPowerup(t7_powerup_timewarp, v)
				ReturnPosition("Returning" .. "timewarp", math.Round(v - ctime))	
			end

			if k == "infinite" then
				AddPowerup(t7_powerup_infinite, v)
