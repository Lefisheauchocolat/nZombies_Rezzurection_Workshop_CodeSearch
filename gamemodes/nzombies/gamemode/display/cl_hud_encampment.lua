...
local function ScoreHud_t6_dlc()
	...
	for k, v in ipairs(plytab) do
		...
		surface.DrawTexturedRect(wr + (istheplayer and 55 or 45), h - (istheplayer and 170 or 185)*scale - offset, 210*scale, 50*scale)
		...
		surface.DrawTexturedRect(wr + 10*scale, h - (istheplayer and 170 or 185)*scale - offset, (istheplayer and 48 or 40)*scale, (istheplayer and 48 or 40)*scale)

		surface.SetDrawColor(pcolor)
		surface.DrawOutlinedRect(wr + 10*scale, h - (istheplayer and 170 or 185)*scale - offset, (istheplayer and 50 or 42)*scale, (istheplayer and 50 or 42)*scale, 2)
		...
		if v.GetShovel and IsValid(v:GetShovel()) then
			...
			surface.SetDrawColor(color_white)
			surface.DrawTexturedRect(wr + (36*scale), h - ((istheplayer and 152 or 175)*scale) - offset, 32, 32)
		end
		...
		if nz_shownames:GetBool() then
			...
			if v:IsSpeaking() then
				...
				surface.SetMaterial(icon)
				surface.SetDrawColor(color_white)
				surface.DrawTexturedRect((wr + tw) + 16, h - (190*scale) - offset - 16, 32, 32)
			end

			draw.SimpleTextOutlined(nick, fontsmall, wr + 10*scale, h - (190*scale) - offset, pcolor, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, color_black)
...
local function PowerUpsHud_t6_dlc()
	...
	for k,v in pairs(nzPowerUps.ActivePowerUps) do	
		if nzPowerUps:IsPowerupActive(k) then
			...
			if k == "timewarp" then
				AddPowerup(t6_powerup_timewarp, v)
				ReturnPosition("Returning" .. "timewarp", math.Round(v - ctime))	
			end

			if k == "infinite" then
				AddPowerup(t6_powerup_infinite, v)
...
local function PerksHud_t6_dlc()
		...
		if nzRound:InProgress() or (#perks > 0) then
			...
			for i=1, nz_perkmax:GetInt() do
				if not ply:HasUpgrade(perks[i]) then
					surface.DrawTexturedRect(w + num_b*(size + 15)*scale, h - 96*scale - 64*row_b, 52*scale, 52*scale)
				end
	...
	for _, perk in pairs(perks) do
		...
		surface.SetMaterial(icon)
		surface.SetDrawColor(color_white)
		surface.DrawTexturedRect(w + num*(size + 15)*scale, h - 96*scale - 64*row, 52*scale, 52*scale)
		...
		if perk == "vulture" and ply:HasVultureStink() then
			...
			surface.SetDrawColor(color_white)
			surface.DrawTexturedRect((w + num*(size + 15)*scale) - 24*scale, (h - 96*scale - 64*row) - 24*scale, 100*scale, 100*scale)
