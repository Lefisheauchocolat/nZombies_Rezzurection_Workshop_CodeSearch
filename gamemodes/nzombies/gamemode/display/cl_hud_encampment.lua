			...
			if v:HasPerk("pop") then
				...
				if delay > CurTime() then
					...
					if fadefac > 0 then
						...
						surface.DrawTexturedRect(wr + 10*scale + (16*scale), h - 185*scale + (16*scale) - offset, 24*scale, 48*scale)
					end
				end
			end
...
local function PerksHud_t6_dlc()
	...
	for i, perk in pairs(perks) do
		...
		surface.DrawTexturedRect(w + num*(size + 15)*scale - fuckset*scale, h - (96 + fuckset)*scale - (64*row)*scale, 52*pulse*scale, 52*pulse*scale)
...
local function ResetRoundPos()
	...
	round_posdata[5] = wscale + 20*scale
	round_posdata[6] = h - 150*1.1*scale
end
...
local function GameBeginRound(round)
	...
	round_posdata[4] = h/2 + 17*scale

	round_posdata[5] = w/2 - (42*1.1*scale)
	round_posdata[6] = h/2 + 35*scale
...
local function RoundHud_t6_dlc()
	...
	if round_intro then
		...
		color = ColorAlpha(color_white, round_alpha)
		surface.SetDrawColor(color)
	end
	...
	if intro_time < CurTime() and kys_time > CurTime() then
		...
		round_posdata[4] = Lerp(kysratio, h - 150*scale, h/2 + 17*scale)

		round_posdata[5] = Lerp(kysratio, wscale + 20*scale, w/2 - (42*1.1*scale))
		round_posdata[6] = Lerp(kysratio, h - 150*1.1*scale, h/2 + 35*scale)
	end
	...
	if round_num <= 5 and round_num > 0 then
		...
		for i=1, string_len(round_num) do
			...
			surface.DrawTexturedRect(round_posdata[5] + 66*1.2*(i-1)*scale, round_posdata[6], 84*1.1*scale, 120*1.1*scale)
		end
	end
end
