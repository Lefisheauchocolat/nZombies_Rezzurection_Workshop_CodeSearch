...
//t7 hud
local t7_hud_dpad_base = Material("nz_moo/huds/t7_prison/uie_t7_zm_hud_ammo_dpadelement_prison.png", "unlitgeneric smooth")
...
local t7_icon_gum = Material("nz_moo/huds/t7_zod/uie_t7_zm_hud_ammo_dpadicnbblgm_new.png", "unlitgeneric smooth")
...
net.Receive("nz_points_notification_prison_hd", function()
...
local Circles = {
	[1] = {r = -2, col = Color(255,200,150,50), colb = Color(10,150,200,100), cole = Color(120,80,0,100)},
	...
	[7] = {r = 4, col = Color(255,200,150,50), colb = Color(10,150,200,100), cole = Color(120,80,0,100)},
}
...
local function InventoryHUD_t7_zod()
	...
	if gumdata and (nzGum:IsUseBaseGum(ply) or nzGum:IsRoundBaseGum(ply)) then
		...
		surface.DrawTexturedRect(w - 168*scale, h - 246*scale, 64*scale, 64*scale)
		...
		if uses > 0 then
			if uses > 9 then
				draw.SimpleTextOutlined(uses, ammofont, w - 137*scale, h - 256*scale, color_t7, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_t7_outline)
			else
				surface.SetMaterial(t7_hud_ammo[uses] or zmhud_icon_missing)
				surface.SetDrawColor(color_white)
				surface.DrawTexturedRect(w - 168*scale, h - 274*scale, 64*scale, 64*scale)
			end
		end
	end
	...
	for _, wep in pairs(plyweptab) do
		...
		if wep.IsTFAWeapon and wep.NZSpecialCategory == "trap" then
			...
			if ammo > 0 and not wep.TrapCanBePlaced then
				...
				if ammocount > 0 then
					...
					surface.SetDrawColor(color_white)
					surface.DrawTexturedRect(w - 88*scale, h - 179*scale, 64*scale, 64*scale)
				end
			else
				surface.SetMaterial(t7_icon_trap_fill)
				surface.SetDrawColor(color_white)
		...
		if wep.IsTFAWeapon and wep.NZSpecialCategory == "shield" and wep.NZHudIcon and not wep.ShieldEnabled then
			...
			if IsValid(wep) and wep.NZHudIcon then
				icon = wep.NZHudIcon_t7zod or wep.NZHudIcon
			end
			if not icon or icon:IsError() then
				icon = zmhud_icon_missing
			end
	...
	for k, v in pairs(PointsNotifications) do
		...
		if v.amount >= 0 then
			if v.profit and v.profit > 0 then
				local pvcol = Entity(v.profit):GetPlayerColor():ToColor()
...
local function GunHud_t7_zod()
	...
	if IsValid(wep) then
		...
		elseif not hudsword[class] then
			if wep.Primary then
			...
			if emptycliptime > CurTime() then
				...
				cwimage("nz_moo/huds/t7/sparks_bw/" .. (math.ceil(CurTime()*30) % 28) .. ".png", w - 324*scale, h - 200*scale, 108*scale*1.4, 168*scale*1.4, ColorAlpha(color_t7zod, 400*alphascale))
			end
			if emptyclip2time > CurTime() then
				...
				cwimage("nz_moo/huds/t7/sparks_bw/" .. (math.ceil(CurTime()*30) % 28) .. ".png", w - 409*scale, h - 200*scale, 108*scale*1.4, 168*scale*1.4, ColorAlpha(color_t7zod, 450*alphascale))
			end
			...
			if wep.CanBeSilenced then
				...
				surface.SetMaterial(icon)
				surface.SetDrawColor(color_t7)
				surface.DrawTexturedRect(w - 208*scale, h - 188*scale, 42*scale, 42*scale)

				local ammoTotal2 = ply:GetAmmoCount(wep:GetSecondaryAmmoType()) + (wep.Clip3 and wep:Clip3() or wep:Clip2())
				if ammoTotal2 > 0 then
					draw.SimpleTextOutlined(ammoTotal2, (fontsmall), w - 182*scale, h - 132*scale, color_t7, TEXT_ALIGN_RIGHT, TEXT_ALIGN_BOTTOM, 2, color_t7_outline)
				end
			end
	...
	if ply:ShouldDrawInventoryHUD() then
		...
		if numspecial > 0 then
			...
			if tacnade and IsValid(tacnade) and tacnade.NZHudIcon then
				icon = tacnade.NZHudIcon_t7zod or tacnade.NZHudIcon
			end
			...
			for i = numspecial, 1, -1 do
				...
				surface.SetDrawColor(ColorAlpha(LerpVector(i/4, color_t7:ToVector(), color_t7zod:ToVector()):ToColor(), 255/i))
		...
		if num > 0 then
			...
			for i = num, 1, -1 do
				...
				surface.SetDrawColor(ColorAlpha(LerpVector(i/5, color_t7:ToVector(), color_t7zod:ToVector()):ToColor(), 255/i))
...
local function PerksHud_t7_zod()
	...
	if perkflashtime > CurTime() then
		...
		cwimage("rf/round/sparks/" .. (math.ceil(CurTime()*30) % 20) .. ".png", (w + 46*scale) + ((flashcount-1)*(size + 6))*scale, h - 154*scale - 64*flashrow, 168*scale, 252*scale, ColorAlpha(color_white, 255*alpha))
...
local function ZedCounterHUD_t7_zod()
...
-- Hooks
hook.Add("HUDPaint", "nzHUDswapping_t7_prison", function()
	if nzMapping.Settings.hudtype == "Mob of the Dead (HD)" then
		...
		hook.Add("HUDPaint", "0nzhudlayer", GunHud_t7_zod )
		hook.Add("HUDPaint", "1nzhudlayer", InventoryHUD_t7_zod )
		hook.Add("HUDPaint", "zedcounterHUD", ZedCounterHUD_t7_zod )
