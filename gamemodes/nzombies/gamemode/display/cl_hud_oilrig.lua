...
local color_blood = Color(90, 0, 0, 255)
...
//--------------------------------------------------/Jbird, StevieWonder, GhostlyMoo and Fox's Oilrig HUD\------------------------------------------------\\

//t5 hud
local t5_hud_dpad_outer_frame = Material("nz_moo/huds/oilrig/ammo_background.png", "unlitgeneric smooth")
...
local t5_hud_dpad_lines_fade = Material("nz_moo/huds/oilrig/ammo_background2_new.png", "unlitgeneric smooth")
...
local t5_icon_grenade = Material("nz_moo/huds/oilrig/hud_us_grenade.png", "unlitgeneric smooth")
...
net.Receive("nz_points_notification_oilrig", function()
...
local function GunHud_t5()
	...
	surface.SetDrawColor(color_white)
	surface.DrawTexturedRect(w - 402*scale, h - 112*scale, 152*scale*1.6, 46*scale*1.6)
	...
	if nz_showcompass:GetBool() then
		...
		surface.SetDrawColor(color_white)
		surface.DrawTexturedRectRotated(w - (128*scale), h - 106*scale, 128*scale*1.8, 128*scale*1.8, angle)
	end
		...
		elseif not illegalspecials[wep.NZSpecialCategory] then
			...
			if wep.CanBeSilenced then
				...
				if ammoTotal2 > 0 then
					draw.SimpleTextOutlined(ammoTotal2, ammo2font, w - 170*scale, h - 90*scale, color_white, TEXT_ALIGN_RIGHT, TEXT_ALIGN_BOTTOM, 2, color_black_100)
				end
			end
			...
			if ply:HasPerk("mulekick") then
				...
				surface.DrawTexturedRect(w - 240*scale, h - 195*scale, 35*scale, 35*scale)
			end
		end
	end
	...
	if num > 0 then
		...
		for i = num, 1, -1 do
			...
			surface.DrawTexturedRect(w - 280*scale + i*(10*scale), h - 38*scale, 36*scale, 36*scale)
		end
	end
...
local function PerksMMOHud_t5()
	...
	for k, v in pairs(ply:GetPerks()) do
		...
		surface.DrawTexturedRect(w - 240*scale - (40*traycount*scale), h - 195*scale, 35*scale, 35*scale)
		...
		elseif mmohud.style == "count" then
			draw.SimpleTextOutlined(ply:GetNW2Int(tostring(mmohud.count), 0), ChatFont, w - 205*scale - (40*traycount*scale), h - 155*scale, fontColor, TEXT_ALIGN_RIGHT, TEXT_ALIGN_BOTTOM, 1, color_black)
		elseif mmohud.style == "%" then
			...
			if (not mmohud.hide) or (mmohud.hide and perkpercent < 100) then
				draw.SimpleTextOutlined(perkpercent.."%", ChatFont, w - 205*scale - (40*traycount*scale), h - 155*scale, fontColor, TEXT_ALIGN_RIGHT, TEXT_ALIGN_BOTTOM, 1, color_black)
			end
		elseif mmohud.style == "chance" then
			draw.SimpleTextOutlined(ply:GetNW2Int(tostring(mmohud.count), 0).."/"..mmohud.max, ChatFont, w - 205*scale - (40*traycount*scale), h - 155*scale, fontColor, TEXT_ALIGN_RIGHT, TEXT_ALIGN_BOTTOM, 1, color_black)
		end
...
local function PowerUpsHud_t5()
	...
	for k,v in pairs(nzPowerUps.ActivePowerUps) do	
		if nzPowerUps:IsPowerupActive(k) then
			...
			if k == "timewarp" then
				AddPowerup(t5_powerup_timewarp, v)
				ReturnPosition("Returning" .. "timewarp", math.Round(v - ctime))	
			end

			if k == "infinite" then
				AddPowerup(t5_powerup_infinite, v)
...
-- Hooks
hook.Add("HUDPaint", "nzHUDswapping_oilrig", function()
	if nzMapping.Settings.hudtype == "Industrial Estate" then
