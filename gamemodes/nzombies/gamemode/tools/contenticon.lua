...
function OpenWeaponSelectMenu(weaponselectentry, weaponadd)
	...
	for _, _wep in pairs(WeaponList) do
		if _wep.Spawnable and not table.HasValue(WeaponCategoryList, _wep.Category) then
	...
	for _, category in ipairs(WeaponCategoryList) do
		...
		Node.DoClick = function()
			...
			for _, _wep in pairs(WeaponList) do
				if _wep.Spawnable and _wep.Category == category then
					local wepData = weapons.Get(_wep.ClassName)
					if wepData and not (nzConfig.WeaponBlackList[_wep.ClassName] or _wep.PrintName == "Scripted Weapon" or wepData.NZPreventBox or wepData.NZTotalBlacklist) then
						if wepData.SubCategory then
							categorizedWeapons[wepData.SubCategory] = categorizedWeapons[wepData.SubCategory] or {}
							table.insert(categorizedWeapons[wepData.SubCategory], _wep)
						else
			...
			for subCategory, weapons in SortedPairs(categorizedWeapons) do
				...
				for _, wep in ipairs(weapons) do
					...
					icon.DoClick = function()
						...
						weaponselectentry:SetText((wep.PrintName or wep.ClassName) .. special)
						weaponselectentry.optionData = wep.ClassName
