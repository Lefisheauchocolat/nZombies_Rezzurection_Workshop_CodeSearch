...
nzTools:CreateTool("zombie_block", {
	...
	PrimaryAttack = function(wep, ply, tr, data)
		local ent = tr.Entity
		if IsValid(ent) and ent:GetClass() == "zombie_wall_block" then
			nzMapping:ZombieBlockSpawn(ent:GetPos(),ent:GetAngles(), data.model, nil, ply)
			ent:Remove()
		else
			nzMapping:ZombieBlockSpawn(tr.HitPos,Angle(90,(tr.HitPos - ply:GetPos()):Angle()[2] + 90,90), data.model, nil, ply)
		end
	end,
	SecondaryAttack = function(wep, ply, tr, data)
		if IsValid(tr.Entity) and tr.Entity:GetClass() == "zombie_wall_block" then
			tr.Entity:Remove()
		end
	end,
	...
	displayname = "Invisible Zombie Block Spawner",
	desc = "LMB: Create Invisible Zombie Block, RMB: Remove Invisible Zombie Block, R: Change Model",
	...
	interface = function(frame, data, context)
		...
		function Scroll.UpdateData(data)
			nzTools:SendData(data, "zombie_block", data) -- Save the same data here
		end
	...
	defaultdata = {
		model = "models/nzombies_plates/plate3x3.mdl"
	},
})

nzTools:EnableProperties("zombie_block", "Edit Model...", "icon16/brick_edit.png", 9009, true, function( self, ent, ply )
	if ( !IsValid( ent ) or !IsValid(ply) ) then return false end
	if ( ent:GetClass() != "zombie_wall_block" ) then return false end
