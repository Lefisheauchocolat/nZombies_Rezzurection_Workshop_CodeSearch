	...
	for k,v in pairs(ply:GetCarryItems()) do
		...
		if item then
			if item.model then
				...
				surface.DrawTexturedRect(ScrW() - 400*scale - num*40*scale, ScrH() - 100*scale, 30*scale, 30*scale)
				if item.icon then
					surface.SetMaterial(item.icon)
					surface.DrawTexturedRect(ScrW() - 384*scale - num*40*scale, ScrH() - 100*scale, 16*scale, 16*scale)
				end
