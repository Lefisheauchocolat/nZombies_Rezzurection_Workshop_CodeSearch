...
function nzRound:EnableSpecialFog( bool )
	...
	if bool and (!specialfog or !foginit) then
		if IsValid(ent) then
			tfogstart = ent:GetSpecialFogStart()
			tfogend = ent:GetSpecialFogEnd()
			tfogdensity = ent:GetSpecialDensity()
			tfogcolor = ent:GetSpecialFogColor()
		else
			tfogstart = 50
			tfogend = 1000
			tfogdensity = 0.9
...
function nzFogThink()
	...
	if IsValid(ent) then
		if specialfog then
			fogstart = ent:GetSpecialFogStart()
			fogend = ent:GetSpecialFogEnd()
			fogdensity = ent:GetSpecialDensity()
			fogcolor = ent:GetSpecialFogColor()
		else
