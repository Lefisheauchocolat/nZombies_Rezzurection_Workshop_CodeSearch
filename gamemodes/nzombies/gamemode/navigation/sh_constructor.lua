...
nzNav.Reserved = nzNav.Reserved or {} -- Chtidino added this one.
...
-- Moo Mark 2/25/23: Nav Locks now work like how they do nZu, which also means any changes done are handled by themselves and don't have to rely on the zombies having a modified path generator to work.

function nzNav.Functions.NavLockApply() -- You see... While I wanna kill 2016 Zet0r for removing the Nav Group system... I'll Gladly give 2019 Zet0r a pat the back for how he did nav locks in nZu, so I'm gonna rob him!
	navdoors = {}
	for k,v in pairs(navmesh.GetAllNavAreas()) do
		local id = v:GetID()
		if nzNav.Locks[id] then
			if nzNav.Locks[id].locked and nzNav.Locks[id].link then
				local group = nzNav.Locks[id].link
				...
				--print("Locked has a link")
			elseif nzNav.Locks[id].locked and !nzNav.Locks[id].link then
				-- Just disconnect all outgoing connections
				for k2,v2 in pairs(v:GetAdjacentAreas()) do
					v:Disconnect(v2)
					--print("Locked no link")
				end
			end
		end
	end
...
hook.Add("OnDoorUnlocked", "UnlockthatShit", function(ent, link, ply)
	...
	--print("Door Bought")
	if navdoors and navdoors[link] then
		--print("you shat")
		for k,v in pairs(navdoors[link]) do
			if IsValid(v.Area) then
				--print("I shat")
				for k2,v2 in pairs(v.Connections) do
					v.Area:ConnectTo(v2)
					--print("Door Unlocked! Nav Reconnected.")
				end
			end
		end
	end
