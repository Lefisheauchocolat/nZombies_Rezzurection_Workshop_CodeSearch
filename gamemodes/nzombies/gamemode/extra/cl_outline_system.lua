...
hook.Add("SetupOutlines", "nzrOutlines", function()
	...
	if ply:Alive() then
		...
		for _, ent in pairs(ents.FindInSphere(ply:GetPos(), 400)) do
			if ent:GetNWString('OutlineType') == "armorplate" then
				tabWhite[#tabWhite+1] = ent
			elseif ent:GetNWString('OutlineType') == "salvage" then
				tabGreen[#tabGreen+1] = ent
			elseif ent:GetNWString('OutlineType') == "killstreak" then
				tabPurple[#tabPurple+1] = ent
			end
		end

		outline.Add(tabWhite, Color(200,200,200), 2)
		outline.Add(tabGreen, Color(50,150,0), 2)
		outline.Add(tabPurple, Color(120,0,200), 2)
	end
end)